...
int main(){
    ...
    for(i=0;i<number;i++){
        printf("DWSE TON ARI8MO GIA EISAGWGH STH ARXH TH LISTA: ");
        scanf("%d", &Item);
        PrevPtr = NULL; //gia na diatrexei kathe fora, thewreitai arxiki timi, dld i arxi tis listas
        LinkedInsert(&List, Item, PrevPtr);
    }
...
void delete_n_element(ListPointer *List, int n){
    ...
    if(!EmptyList(*List)){
            ...
            if(n==1){ //an einai to 1o stoixeio
                TempPtr = *List;
                *List = TempPtr->Next;
            }
            else{
