...
boolean IdenticalBST(BinTreePointer Root1, BinTreePointer Root2);
...
int main(){
    ...
    char identical[9] = "identical", different[9]="different";

    int i;

    for(i=0;i<9;i++){
        RecBSTInsert(&ARoot, identical[i]);
        RecBSTInsert(&BRoot, identical[i]);
        RecBSTInsert(&CRoot, different[i]);
    }

    if(IdenticalBST(ARoot,BRoot)==TRUE){
        printf("Ta dyadika dentra A kai B einai idia\n");
    }
    else{
        printf("Ta dyadika dentra A kai B diafferoun\n");
    }
    if(IdenticalBST(BRoot,CRoot)==TRUE){
...
boolean IdenticalBST(BinTreePointer Root1, BinTreePointer Root2){
    ...
    if(BSTEmpty(Root1) && BSTEmpty(Root2)){
        ...
        if(BSTEmpty(Root1)!=TRUE && BSTEmpty(Root2)!=TRUE){
            return (Root1->Data==Root2->Data) && IdenticalBST(Root1->LChild, Root2->LChild)&& IdenticalBST(Root1->RChild, Root2->RChild);
        }
        else{
            return FALSE;
        }
